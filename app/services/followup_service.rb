class FollowupService
  def self.check_and_create
    ...
    Contact.find_each do |contact|
      ...
      next if contact.followups.where(sequence_number: sequence).exists?
      ...
      if Setting.followup_use_ai?
        ...
        draft = Draft.create!(
          ...
          status: Setting.auto_send_followups? ? :reviewed : :draft
        )
        followup.update!(draft: draft)

        if Setting.auto_send_followups? && draft.reviewed?
