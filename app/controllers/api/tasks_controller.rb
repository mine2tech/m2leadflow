module Api
  class TasksController < BaseController
    ...
    def claim
      rows = Task.where(id: @task.id, status: :pending).update_all(status: :claimed, updated_at: Time.current)
      if rows == 0
        @task.reload
    ...
    def complete
      ...
      ActiveRecord::Base.transaction do
        result_data = params[:result].respond_to?(:to_unsafe_h) ? params[:result].to_unsafe_h : (params[:result] || {})
    ...
    def process_task_result(task)
      ...
      when "classify_reply"
        TaskResultProcessors::ClassifyReply.call(task)
      end
    end
  end
end
